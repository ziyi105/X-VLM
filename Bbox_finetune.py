...
from dataset.utils import collect_tensor_result, bbox_finetune_eval
...
os.environ["WANDB_API_KEY"] = "f4e1162a7f5fd36d0bd2979e36cd5b370f84a706" 
...
def val(model, data_loader, tokenizer, device):
    ...
    for image, text, bbox, image_path in metric_logger.log_every(data_loader, print_freq, header):
        ...
        for image_path, coord in zip(image_path, outputs_coord):
            result.append({'image': image_path, 'pred': coord})
    wandb.log({
...
def main(args, config):
    ...
    grd_train_dataset, grd_test_dataset = create_dataset('bbox_finetune', config, args.evaluate)
    ...
    if args.evaluate:
        ...
        results = collect_tensor_result(result, filename='bbox_finetune_eval', local_wdir=args.result_dir,
                                        hdfs_wdir=args.output_hdfs,
                                        write_to_hdfs=world_size > 8)

        if utils.is_main_process():
            grounding_acc = bbox_finetune_eval(results, grd_test_dataset)

            wandb.log({
                "accuracy": grounding_acc.get('accuracy', 0),
                "mean_iou": grounding_acc.get('mean_iou', 0)
            })
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output_hdfs", type=str, default="", help="HDFS directory to save results (optional)")
