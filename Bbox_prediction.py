...
image_path = '/content/drive/MyDrive/fyp-dataset/test_images/cropped_scene_150.jpeg'  # Replace with your image path
...
pred_bbox_abs = [
    int(pred_bbox[0] * W),  # x_min
    int(pred_bbox[1] * H),  # y_min
    int(pred_bbox[2] * W),  # x_max
    int(pred_bbox[3] * H)   # y_max
]
print("Predicted Bounding Box:", pred_bbox_abs)

x_min, y_min, width, height = pred_bbox_abs
x_max = x_min + 14
y_max = y_min + 14
...
rect = patches.Rectangle((x_min, y_min), 14, 14, linewidth=2, edgecolor='r', facecolor='none')
ax.add_patch(rect)
