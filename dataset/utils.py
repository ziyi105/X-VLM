...
def bbox_finetune_eval(results, dataset, iou_threshold=0.5):
    correct = 0
    total = 0
    iou_scores = []
    ...
    for res in tqdm(results):
        ...
        image = res['image']
        pred_bbox = res['pred']  # Predicted bounding box [center_x, center_y, width, height] (normalized)

        # Get ground truth bounding box and image dimensions
        ground_truth = dataset.get_ground_truth(image)
        gt_bbox = ground_truth['bbox']  # Ground truth bounding box [x, y, w, h]
        image_width = ground_truth['width']
        image_height = ground_truth['height']

        # Convert predicted bounding box from normalized [center_x, center_y, width, height]
        # to absolute [x, y, w, h]
        pred_bbox[0] *= image_width  # center_x -> absolute
        pred_bbox[1] *= image_height  # center_y -> absolute
        pred_bbox[2] *= image_width  # width -> absolute
        pred_bbox[3] *= image_height  # height -> absolute

        pred_bbox[0] -= pred_bbox[2] / 2  # Convert center_x to top-left x
        pred_bbox[1] -= pred_bbox[3] / 2  # Convert center_y to top-left y
    ...
    # Convert mean_iou to a Python float if it's a tensor
    if isinstance(mean_iou, torch.Tensor):
